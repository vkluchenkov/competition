...
export const GridVideo: React.FC<GridVideoProps> = ({ video }) => {
  ...
  return (
    <div id={video.id} css={styles.video}>
      <div
      ...
      >
        <Link to={`single/${video.id}`}>
          <img src={video.image} alt={video.title} css={styles.video_img}></img>
        </Link>
        <div css={styles.video_time}>{videoDuration}</div>
      ...
      <div css={styles.video_description}>
        <div css={styles.video_avatar}>{video.avatar}</div>
        <div>
          <Link to={`single/${video.id}`} style={styles.link}>
            <h5 css={styles.video_title}>{video.title}</h5>
          </Link>
          <p css={styles.video_channel}>{video.channel.name}</p>
          <p css={styles.video_meta}>
            {videoViews} • {videoAge}
          </p>
        </div>
      </div>
